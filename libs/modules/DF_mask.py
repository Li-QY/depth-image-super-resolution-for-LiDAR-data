...
class Generator(nn.Module):
    def __init__(self, in_ch, out_ch, scale_factor):
        ...
        self.block1D = conv3x3(in_ch, 64, kernel_size=9, padding=4, stride=1)
        self.block1_1D = nn.PReLU()
        self.block1F = nn.Conv2d(in_ch, 64, kernel_size=9, padding=4, stride=1)
        self.block1_1F = nn.PReLU()

        block2 = self._make_layer(ResidualBlock, 128, 5)
        self.block2 = nn.Sequential(*block2)

        self.block3 = conv3x3(128, 64, kernel_size=3, padding=1, stride=1)
    ...
    def forward(self, x, y, mask):
        ...
        blockD1, mask = self.block1D(blockD, mask)
        blockD1 = self.block1_1D(blockD1)
        blockF1 = self.block1F(blockF)
        blockF1 = self.block1_1F(blockF1)
        block1 = torch.cat((blockD1, blockF1), dim=1)
        block2 = block1
        for m in self.block2:
            block2, mask = m(block2, mask)
        block3, mask = self.block3(block2, mask)
        block3 = self.block3_1(block3)
        block4 = blockD1 + block3
...
class ResidualBlock(nn.Module):
    def __init__(self, channels):
        ...
        self.conv1 = conv3x3(128, channels, kernel_size=3, padding=1, stride=1)
        self.bn1 = nn.BatchNorm2d(channels)
        self.prelu = nn.PReLU()
        self.conv2 = conv3x3(channels, channels, kernel_size=3, padding=1, stride=1)
