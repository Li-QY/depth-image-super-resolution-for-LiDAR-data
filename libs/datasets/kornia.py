...
class DenseMPO(VisionDataset):
    ...
    def _npy(self, npy_path):
        ...
        data = transforms.Resize(
            data, self.transform.base_size, self.transform.interp_mode
        )
        return data
...
class Augmentation:
    def __init__(self, device, transform):
        ...
        self.vmin, self.vmax = (
            [0, 1.0] if not self.transform.vrange else self.transform.vrange
        )  #
        if not isinstance(self.transform.crop_size, Sequence):  # 将传入的数值改为序列
            crop_size = (crop_size, crop_size)
        self.crop_H, self.crop_W = crop_size
        assert isinstance(self.transform.base_size, Sequence)
        self.H, self.W = self.transform.base_size
        ...
        self.flip = self.transform.flip
        self.config = self.transform.config
    ...
    def __call__(self, data):
        ...
        interp_mode = self.transform.interp_mode
        for modal, scan in data.items():
            if self.flip:
                if modal == "mask":
                    scan = self._reformation(scan)
                    scan = F.interpolate(scan, size=(self.H, self.W), mode=interp_mode)
                    scan = self._crop(scan, sh, sw)
                    data[modal] = torch.flip(scan, [4])
                else:
                    scan = self._reformation(self._normalize(scan, modal))
                    scan = F.interpolate(scan, size=(self.H, self.W), mode=interp_mode)
                    scan = self._crop(scan, sh, sw)
                    data[modal] = torch.flip(scan, [4])
            else:
...
class JointPreprocess(object):
    def __init__(
        self, base_size, crop_size, flip, config, crop_mode="center", vrange=None
    ):
    ...
    def __call__(self, scan_dict):
        sh, sw = self._sample_topleft()
        crop_start = [sh, sw]  # crop_start
        interp_mode = self.config.train.interp_mode
